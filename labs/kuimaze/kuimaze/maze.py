...
@author: Otakar Jašek
@contact: jasekota(at)fel.cvut.cz
@copyright: (c) 2017
'''
...
class Maze:
    ...
    def __init__(self, image, grad, node_rewards=None, path_costs=None, trans_probs=None, show_level=SHOW.FULL_MAZE,
        ...
        if self.__node_rewards is None:
            self.__node_rewards = np.zeros(self.__maze.shape, dtype=int)
    ...
    def get_next_states_and_probs(self, state, action):
        ...
        for out_action in ACTION:
            next_state, cost = self.result(state,out_action)
    ...
    def show_and_break(self, drawed_nodes=None):
        ...
        If show_level is L{SHOW.NONE}, this function has no effect
    ...
    def __draw_text_four(self, current_node, my_list):
        ...
        for i in range(4):
            if self.__text_handles_four[x][y][i] > 0:
                if self.__gui_canvas.itemcget(self.__text_handles_four[x][y][i], "text") != format(my_list[i], '.1f'):
                    self.__gui_canvas.itemconfigure(self.__text_handles_four[x][y][i], text=format(my_list[i], '.1f'))
            else:
                center = self.__get_cell_center_coords(x, y)
                size = self.__cell_size/2
                if i == 0:
                    self.__text_handles_four[x][y][i] = self.__gui_canvas.create_text([center[0], center[1] - int(0.7*size)],
                                                                              text=format(my_list[i], '.1f'), font=self.__font_small)
                elif i == 1:
